...
class VideoProcessor:
    ...
    def _load_model(self):
        ...
        try:
            ...
            # Use CUDA provider with CPU fallback
    ...
    def preprocess_frame(self, frame):
        ...
        Returns:
            np.ndarray: Preprocessed frame ready for model input (uint8)
        """
        ...
        # Keep as uint8 (0-255 range) - don't normalize to float
    ...
    def postprocess_detections(self, outputs, conf_threshold=0.3):
        """
        Postprocess model outputs using TI-specific format
        Based on test_optimization.py parse_detection_output function
        ...
        try:
            for output in outputs:
                # Handle different output shapes - TI format is typically (batch, detections, 6)
                        ...
                        if len(detection) >= 6:
                            ...
                            if confidence > conf_threshold:
                                ...
                                detections.append([
                                    ...
                                    float(confidence), int(class_id), class_name
                                ])
                
                # Handle case where output might be 2D
            ...
            print(f"Output content sample: {outputs[0].flatten()[:10] if len(outputs) > 0 else 'N/A'}")
